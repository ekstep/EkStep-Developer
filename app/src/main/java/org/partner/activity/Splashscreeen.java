        ...
        new AsyncTask<Void,Void,Void>(){
            ...
            protected Void doInBackground(Void... voids) {
                try {
                    ...
                    configModel.setPartnerName(jsonObject.getString(Config.partnerName));
                    ...
                    for (int i=0;i<size;i++){
                        ...
                        //if(j==0){ //1st position fixed as sectionHeadings that's why adding displayOrder
                            ...
                            //Instructions
                           /* instructionsFields[i]=new InstructionsField();
                            JSONObject instructionsObject=fieldObject.getJSONObject(Config.instructions);
                        instructionsFields[i].setTitle(instructionsObject.getString(Config.title));
                        instructionsFields[i].setDisplayOrder(instructionsObject.getInt(Config.displayOrder));
                        sectionValues[i].setInstructionsField(instructionsFields[i]);
*/
...
*/
    ...
    public void onFailure(GenieResponse genieResponse) {
        String result = new Gson().toJson(genieResponse.getResult());
        Toast.makeText(mContext,"partner registration failed.",Toast.LENGTH_LONG).show();
        if(D)
